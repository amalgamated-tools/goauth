...
`MagicLinkHandler` emits structured log events via `slog` with the request context for trace correlation. Log events emitted directly by `MagicLinkHandler` methods go through the handler's `Logger` field; when `Logger` is `nil`, `slog.Default()` is resolved at each call site. To route these events to a separate destination, set `Logger` to a `*slog.Logger` backed by the desired handler.
...
The email delivery failure event is `ERROR`-level but does **not** result in an HTTP 500 — `RequestMagicLink` still returns HTTP 200 to prevent email enumeration (see [Token retention on email delivery failure](#token-retention-on-email-delivery-failure)).
