...
A nil `SendEmail` is caught by `Validate()` at startup; `SendVerification` also returns HTTP 503 as a runtime safety net. To skip email delivery in tests, supply a no-op `SendEmail` function instead.
...
`SendVerification` returns HTTP 400 for a missing `email` field or a malformed request body, and HTTP 503 if `SendEmail` is `nil`. Beyond those cases, it returns HTTP 200 regardless of whether the address is registered:
...
|---|---|---|
| `SendVerification` | 200 OK | Always, except for the 400 and 503 cases (even if email is unregistered or already verified) |
...
!!! info "Silent success for unregistered / already-verified addresses"
    `SendVerification` returns 200 for unregistered emails and already-verified addresses without storing a token or sending an email. Only the missing-`email` validation check (400) and a nil `SendEmail` misconfiguration (503) surface as non-200 errors.
