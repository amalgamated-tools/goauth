...
The email delivery failure event is `ERROR`-level but the handler still returns HTTP 200 (see [Token cleanup on email delivery failure](#token-cleanup-on-email-delivery-failure)). The token consumption failure event (`"password reset: consume token"`) is also `ERROR`-level but does not abort the response — the password has already been updated and HTTP 200 is returned. All other `ERROR`-level events are followed by an HTTP 500 response.
