...
- Log output uses the `slog.Logger` that is the process-wide default **at the time each log event is emitted** (i.e. `slog.Default()` is resolved lazily). You can replace the default logger after calling `StartCleanup` and subsequent log events will use the new logger.
