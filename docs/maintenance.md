...
`StartCleanup` emits structured log events via `slog.ErrorContext`, propagating the context passed to `StartCleanup` for trace correlation. All log output goes through `slog.Default()` resolved at the time of each log entry.
