...
- Log output uses the `slog.Logger` that is the process-wide default **at the time each log line is emitted** (i.e. `slog.Default()` is resolved inline at every log site, not captured once at startup). This means `slog.SetDefault()` calls made after `StartCleanup` returns are honoured by subsequent cleaner runs.
