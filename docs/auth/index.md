...
| `auth.ErrOIDCSubjectAlreadyLinked` | Exported sentinel indicating an OIDC subject is already linked to a user. Suppressed only in the best-effort login path; the interactive link callback treats any non-nil return from `LinkOIDCSubject` as a failure (redirects with `oidc_link_error=Failed+to+link` for `OIDCHandler`, or `oauth2_link_error=Failed+to+link` for `OAuth2Handler`). Prefer an idempotent upsert returning `nil` over returning this sentinel — see [Store Interfaces](store-interfaces.md#userstore) |
